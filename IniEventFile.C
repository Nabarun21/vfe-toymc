...
TString IniEventFile(TString origPulse,TString method)
{
  TString fileinput ="data/";
  fileinput+=origPulse;
  TFile *fin = new TFile(fileinput);

  TString pulsename="gr";
  pulsename+=method;
  //  pulsename+=shapingtime;
  cout<<"pulsename "<<pulsename<<endl;
  TGraph *grPulseShape = (TGraph*)fin->Get(pulsename);
  grPulseShape->SetName("grPulseShape");
  grPulseShape->SetTitle("Pulse shape");

  const TString fileoutput      = "data/inieventfile.root";
  TFile *fout = new TFile(fileoutput.Data(),"recreate");
