...
double * extractsigma(TString samples_file,TString inieventfile,int iter,TCanvas* canv1,int& canvaspad,int nsmpl=NSAMPLES,int nfrq=NFREQ,int peakphase=150) 
{
  ...
  //  Pulse pSh(inieventfile,nsmpl,nfrq);
  ...
  //    if (iter!=0)
   if (iter%4==0 && iter>0)
	...
	gr->SetPointError(i, 0., 0.044);  // 44 MeV for all samples
      ...
      // Fit 
      canv1->cd(canvaspad);
      fPulseShape->SetParameters(guja, 0., amplitudeTruth);
      fPulseShape->SetLineColor(2);
      
      char grname[120];
      sprintf(grname,"pilepup_%i_event_%i",iter*10,5*iter);    
      ...
      canv1->Update();
      canvaspad+=1;
      cout<<"Max fit y value  "<<fPulseShape->GetMaximum()<<endl;
     
      // if(0<fPulseShape->GetMaximumX()&&fPulseShape->GetMaximumX()<500)

      //{ 
      //     cout<<"Max fit X value  "<<fPulseShape->GetMaximumX()<<endl;
      // cout<<"Max fit X value  "<<fPulseShape->GetMaximum()<<endl;
      // cout<<"Phase "<<IDSTART-iter<<endl;
	  //}
